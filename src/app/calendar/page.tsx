...
/** スライダー値から色を算出（保存色がないときのフォールバック） */
function colorFromSlider(value: number): string {
...
export default function Calendar() {
  ...
  /** 保存色があればそれを優先／無ければスライダーから推定／それも無ければグレー */
  ...
  const handleMonthChange = (month: string) => {
    setCurrentMonth(moment(month, 'YYYY-MM'))
  ...
  const calendar = useMemo(() => {
    ...
    while (day.isBefore(endDay, 'day')) {
      weeks.push(Array(7).fill(null).map(() => day.add(1, 'day').clone()))
    }
    return weeks
  }, [currentMonth])
            ...
            {isDropdownOpen && (
              <div className="absolute top-12 left-1/2 transform -translate-x-1/2 bg-white shadow-lg rounded-lg z-10 w-auto px-12">
                {months.map((m) => (
                  ...
                  >
                    {moment(m, 'YYYY-MM').format('YYYY年MM月')}
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="pt-12 grid grid-cols-7 gap-1 ">
              {calendar.map((week, wi) => (
                <React.Fragment key={wi}>
                  {week.map((day) => {
                    const rec = dailyRecords[dayKey(day)]
                    const circleColor = decideCircleColor(rec)
                    return (
                      <div
